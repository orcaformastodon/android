...
internal class PostDetailsConversionScopeTests {
  ...
  fun comments() {
    runPostDetailsConversionTest {
      post().comment.get(page = 0).test {
        awaitItem()
        comment()
        assertThat(awaitItem()).contains(comment)
      }
    }
  }
  ...
  fun uncomments() {
    runPostDetailsConversionTest {
      post().comment.get(page = 0).test {
        ...
        awaitItem()
        uncomment()
        assertThat(awaitItem()).doesNotContain(comment)
      }
    }
  }
  ...
  fun favorites() {
    runPostDetailsConversionTest {
      post().favorite.get(page = 0).test {
        awaitItem()
        favorite()
        assertThatIDsOf(awaitItem()).contains(actor.id)
        unfavorite()
      }
    }
  }
