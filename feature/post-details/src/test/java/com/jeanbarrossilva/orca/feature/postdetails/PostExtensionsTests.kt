...
import com.jeanbarrossilva.orca.feature.postdetails.conversion.runPostDetailsConversionTest
...
internal class PostExtensionsTests {
  ...
  fun convertsPostIntoDetails() {
    runPostDetailsConversionTest {
      assertThat(post().toPostDetails(actor, colors, onLinkClick, onThumbnailClickListener))
        .isEqualTo(
          PostDetails(
            post().id,
  ...
  fun emitsWhenConvertingPostIntoFlowOfDetailsAfterCommentIsAdded() {
    runPostDetailsConversionTest {
      post().comment.count.let { previousCommentCount ->
        post().toPostDetailsFlow(actor, colors, onLinkClick, onThumbnailClickListener).test {
          awaitItem()
          comment()
